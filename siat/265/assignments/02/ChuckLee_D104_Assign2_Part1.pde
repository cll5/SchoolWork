/*****************************************************************
 ...
 In this part of the assignment, there will be six states for
 animating Doraemon. The animation is Doraemon swinging its left arm
 counter clockwise in full a circle.
...
//Global Variable Declarations for the scene method
...
void setup() {
  ...
  //Initialize global variables for the scene method
...
void draw() {
  drawScene();
  drawDoraemon((0.75 * width), (0.8 * height), 0, 0.75, (frameCount/10) % 6);
}
...
void drawScene() {
  ...
  pushMatrix();
    ...
    pushMatrix();
      ...
      //Drawing the glow around the moon
    ...
    //Drawing the three clouds under the moon
...
void drawCloud(float xPos, float yPos, float scaleFactor, int cloudType, int shades) {
  ...
  ***********************************/
  ...
  pushMatrix();
    ...
    //There are two type of clouds to select to draw
    switch(cloudType) {
      case 0:
