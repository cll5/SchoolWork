...
int NUM_DORAEMONS = 8; // number of Doraemons to create in the draw method
...
//Arrays for Doraemon movements
float [] xPos = new float[NUM_DORAEMONS];
float [] yPos = new float[NUM_DORAEMONS];
float [] xVel = new float[NUM_DORAEMONS];
float [] yVel = new float[NUM_DORAEMONS];
float [] scaleFactors = new float[NUM_DORAEMONS];

//Variables for collision detection algorithm
float xBoundary, yBoundary;
...
void setup() {
  ...
  //Initialize Doraemon movement variables with random values
  for(int i = 0; i < NUM_DORAEMONS; i++) {
    xPos[i] = random(DORAEMON_HALF_WIDTH, (width - DORAEMON_HALF_WIDTH));
    yPos[i] = random(DORAEMON_HALF_HEIGHT, (height - DORAEMON_HALF_HEIGHT));
    xVel[i] = random(-5, 5);
    yVel[i] = random(-5, 5);
    scaleFactors[i] = random(0.25, 0.5);
  }
  
  //Initialize collision algorithm variables
  xBoundary = 0.0;
  yBoundary = 0.0;
...
void draw() {
  ...
  for(int i = 0; i < NUM_DORAEMONS; i++) {
    ...
    //If any corners or edge of the bounding box is detected to be outside the screen window size, then reverse the velocity vectors
    
    //Calculate the x- and y- boundaries of the bounding box
    xBoundary = DORAEMON_HALF_WIDTH * scaleFactors[i];
    yBoundary = DORAEMON_HALF_HEIGHT * scaleFactors[i];
    
    //Check if the x-boundary is outside the screen
    if( ((xPos[i] + (DORAEMON_XOFFSET * scaleFactors[i])) < xBoundary) || ((xPos[i] + (DORAEMON_XOFFSET * scaleFactors[i])) > (width - xBoundary)) ){
      xVel[i] = -xVel[i];
    }
    
    //Check if the y-boundary is outside the screen
    if( ((yPos[i] + (DORAEMON_YOFFSET * scaleFactors[i])) < yBoundary) || ((yPos[i] + (DORAEMON_YOFFSET * scaleFactors[i])) > (height - yBoundary)) ){
      yVel[i] = -yVel[i];
    ...
    //Draw Doraemon
    drawDoraemon(xPos[i], yPos[i], DORAEMON_ANGLE, scaleFactors[i], (frameCount/10) % 6);
  }
}
